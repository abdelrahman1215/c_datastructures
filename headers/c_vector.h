...
bool c_vector_add_elment(c_vector *vec_ptr , void *obj_ptr){
    if(!obj_ptr || !vec_ptr){
        return false;
    }
    ...
    if(vec_ptr -> chunks_ptr[vec_ptr -> chunk_no - 1] -> used_size == vec_ptr -> chunk_size){
        ...
        vec_ptr -> chunks_ptr[vec_ptr -> chunk_no] = allocate_mem_chunk(vec_ptr -> chunk_size);
        if(!(vec_ptr -> chunks_ptr[vec_ptr -> chunk_no])){
            return false;
        }
